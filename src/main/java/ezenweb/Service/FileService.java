...
public class FileService {
    ...
    @Autowired
    private HttpServletResponse response; // HTTP로 응답을 보낸 정보와 기능/메소드 가지고 있는 객체
    ...
    public void fileDownLoad(String bfile){
        ...
        // 2. 해당 파일을 객체로 가져오기 [file클래스는 해당 경로의 파일을 객체로 가져와서 다양한 메도스/기능 제공]
        ...
        if(file.exists()) {
            ...
            try{
                ...
                response.setHeader("Content-Disposition", "attacjment;filename="
                        + URLEncoder.encode(bfile.split("_")[1],"utf-8"));


            // 1. 해당파일을 바이트로 불러온다       [BufferedInputStream]
                // 1-1 파일 입력스트림(통신할 때 바이트가 다니는 통로) 객체 생성
            BufferedInputStream fin = new BufferedInputStream(new FileInputStream(file));
                ...
                // 1-2 입력(불러오기) 메소드
                    // 바이트 하나씩 읽어오면서 바이트배열  => 바이트 배열 필요하다
                fin.read(bytes); // 하나씩 바이트를 읽어와서 해당 바이트 배열에 저장 해주는 함수
            ...
            BufferedOutputStream fout = new BufferedOutputStream(response.getOutputStream());
                    // 2-2 응답스트림.write(내보내기할바이트배열) : 내보내기할 바이트배열 준비 상태이면 내보내기
                    fout.write(bytes);
                    // 버퍼 초기화(안전하게
                fin.close(); // 입력 스트림 닫기
