...
public class BoardDao extends Dao {
    ...
    public int getBoardSize(int bcno, String field, String value){
        ...
        try{
            ...
            if(!value.isEmpty()){
                ...
                if(bcno > 0){sql += " and ";} // 카테고리가 있을 때 and로 연결
                else {sql += " where ";} // 카테고리가 없을 때 where로 연결
    ...
    // 5. 글 삭제 처리
    // =============== 머스테치는 컨트롤에서 뷰 반환. ========================== //
    public boolean doDeleteBoard( long bno){
    ...
    public boolean postReplyWrite(Map<String, String> map){
        try {
            String sql = "insert into breply (brcontent,brindex,mno,bno)value(?,?,?,?)";
            ps = conn.prepareStatement(sql);
            ...
            while (rs.next()){
                ...
                map.put("mno", rs.getString("mno"));
                        ...
                        ps.setInt(1, Integer.parseInt(rs.getString("brno")));
                            // (int) 캐스팅=부모, 자식관계이여야한다 -int와 String 상하관계 아니다(형변환) vs Integer.parseInt() : 형변환함수
                            // ************* rs 사용하면 안되는 이유 : 상위 댓글 출력 시 rs 사용중 (while(rs.next))
