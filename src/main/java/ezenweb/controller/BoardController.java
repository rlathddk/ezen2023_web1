...
public class BoardController {
    ...
    public BoardPageDto doGetBoardViewList(@RequestParam int page, @RequestParam int pageBoardSize, @RequestParam int bcno,
    ...
    // 4. 글 수정 처리           /board/update   put      dto
    ...
    public boolean doUpdateBoard(BoardDto boardDto) {
        ...
        if(object != null){
            String mid = (String)object;
            boolean result = boardService.boardWriterAuth(boardDto.getBno(), mid); // 해당 세션정보가 적상한 글인지 체크
            if(result){
    ...
    public boolean doDeleteBoard(@RequestParam int bno){
        ...
        if(object != null){
            String mid = (String)object;
            boolean result = boardService.boardWriterAuth(bno, mid); // 해당 세션정보가 적상한 글인지 체크
            if(result){
    ...
    // 6 .다운로드 처리 (함수만들때 고민할점 1. 매개변수 2.반환 3.사용처)
    ...
    @ResponseBody
    public boolean postReplyWrite(@RequestParam Map<String, String> map){ // map 방식으로 써본거임 dto로 줘도 상관x
        // 1. 현재 로그인된 아이디(세션)
        Object object = request.getSession().getAttribute("loginDto");
    ...
    // 8. 댓글 출력 (brno, brcontent, brdate, brindex, mno) , 매개변수 bno 필요
    @GetMapping("/reply/do")
    @ResponseBody
